...
Promise.all([
  fetch("https://bria9.github.io/infograph-api/neighborhood.geojson").then(
    (response) => response.json()
  ),
  fetch("https://bria9.github.io/infograph-api/foreclosures.json").then(
    (response) => response.json()
  ),
])
  .then(([geoData, foreclosureData]) => {
    ...
    foreclosureData.forEach((f) => {
      ...
      foreclosureCounts[neighborhood] =
        (foreclosureCounts[neighborhood] || 0) + 1;
    });

    // Add foreclosure counts to GeoJSON properties
    geoData.features.forEach((feature) => {
      const neighborhoodName = feature.properties.name;
      feature.properties.foreclosureCount =
        foreclosureCounts[neighborhoodName] || 0;
    });
...
info.update = function (props) {
  this._div.innerHTML =
    "<h4>Louisville Foreclosures</h4>" +
    (props
      ? "<b>" +
        props.name +
        "</b><br />" +
        props.foreclosureCount +
        " foreclosures"
